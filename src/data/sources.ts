...
export const SOURCES: NewsSource[] = [
  ...
  },
  {
    ...
    //
    // This was briefly wired to an unofficial reverse-engineered JSON
    // mirror (dailyhunt.vercel.app) instead, probed the same way RSS
    // candidates are probed above. Reverted: with comingSoon unset, that
    // source was included in every parallel "all sources" fetch — since
    // the aggregate view awaits Promise.all() across every source, one
    // slow or hanging unofficial endpoint stalls the entire homepage
    // load for every visitor, not just people who select DailyHunt
    // specifically. Marking it comingSoon again excludes it from that
    // sweep entirely and restores the honest placeholder explaining the
    // partner-API situation, rather than depending on someone else's
    // side project staying online and fast.
    candidates: [],
    accent: "#2BA84A",
    comingSoon: true,
  },
